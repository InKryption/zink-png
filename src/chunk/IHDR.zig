...
pub const ChunkData = struct {
    ...
    pub fn toBytes(ihdr: ChunkData) [13]u8 {
    ...
    pub const chunk_type: chunk.Type = .IHDR;
    pub fn calculateCrc(ihdr: ChunkData) u32 {
        var hasher = std.hash.Crc32.init();
        hasher.hash(&chunk.Type.string(chunk_type));
...
test "IHDR - ChunkData" {
    ...
    try std.testing.expectEqual(ihdr, ChunkData.fromBytes(ihdr.toBytes()).unwrap() catch @panic("Something here is definitely wrong"));

    var wrong_bytes align(@alignOf(ChunkData)) = ihdr.toBytes();
    ...
    }, ChunkData.fromBytes(wrong_bytes));
}
